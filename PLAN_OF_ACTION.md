...
- Outputs
  ...
  - Updated Helm values files in `bookverse-helm/charts/platform/values-*.yaml` with pinned image tags/digests.
...
```yaml
# bookverse-helm/charts/platform/values-prod.yaml
imageTags:
  inventory: "1.8.3"
...
4) Publish Helm updates
   - Option A: Open a PR in `bookverse-helm` updating `values-*.yaml` (per env).
   - Option B: Dispatch a `platform_release` repository_dispatch event that triggers an existing auto-pin workflow in `bookverse-helm` to apply the changes.
...
sequenceDiagram
  ...
  A->>GH: PR updating values-*.yaml (or dispatch platform_release)
